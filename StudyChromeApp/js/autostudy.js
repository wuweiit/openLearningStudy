...
function mode1() {
    ...
    */
   function getTime() {
      var text = $(window.parent["w_lms_content"].document)
         .find("#tdRemark")
         .text();
    ...
    */
   function getStatus() {
      var html = $(window.parent["w_lms_content"].document).find("#tdRemark")[0]
         .innerHTML;
   ...
   function startStudy() {
      ...
      function study() {
         ...
         if (!($("#StudyStatus").length > 0)) {
            $("body").append(
               "<div id='StudyStatus' style='height: 30px; line-height: 30px;background: #e1dfe2;  width: 100%;  text-align: center;position: fixed; z-index: 11111;font-weight: bold; bottom: 0px;  color: red'>自动正在学习中...</div>"
            );
         }
